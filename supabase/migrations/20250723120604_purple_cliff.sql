...
-- Create trigger to check ban status
...
CREATE INDEX classifications_name_idx ON classifications(name);
CREATE INDEX products_department_id_idx ON products(department_id);
CREATE INDEX formulation_items_position_idx ON formulation_items (formulation_id, item_type, position);
CREATE INDEX productions_display_order_idx ON productions (production_day_id, display_order);
